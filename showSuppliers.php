...
<body>
	                    ...
	                    <div class="ibox-title">
	                    	 <nav class="navbar navbar-static-top" role="navigation" style="margin-bottom: 0">
								<a class="navbar-minimalize minimalize-styl-2 btn btn-primary "
									...
									<h4 class="p-h-sm font-normal"> Supplier Informaiton</h4>
							</nav>
	                    </div>
	                    <div class="ibox-content">
	                        <div id="supplierGrid" style="margin-top:8px"></div>
	                    </div>
	                </div>
	            </div>
        	</div>
    ...
    <form id="form1" name="form1" method="post" action="createSupplier.php">
     	<input type="hidden" id="seq" name="seq"/>
   	</form>
   	 <form id="exportForm" name="exportForm" method="GET" action="Actions/SupplierAction.php">
     	<input type="hidden" id="call" name="call" value="exportSuppliers"/>
	...
	<script type="text/javascript">
	 ...
	 var isSuperUser = "<?php echo $isSuperUser?>";
        ...
        function deleteSuppliers(gridId,deleteURL){
        ...
        function loadGrid(){
            ...
            var source =
            {
                ...
                url: 'Actions/SupplierAction.php?call=getAllSuppliers',
                root: 'Rows',
                cache: false,
                ...
                filter: function()
                {
                    ...
                    $("#supplierGrid").jqxGrid('updatebounddata', 'filter');
                },
                sort: function()
                {
                    // update the grid and send a request to the server.
                    $("#supplierGrid").jqxGrid('updatebounddata', 'sort');
                }
            };
            ...
            $("#supplierGrid").jqxGrid(
            {
                ...
                renderstatusbar: function (statusbar) {
                    ...
                    addButton.click(function (event) {
                        location.href = ("createSupplier.php");
                    });
                    // update row.
                    editButton.click(function (event){
                    	var selectedrowindex = $("#supplierGrid").jqxGrid('selectedrowindexes');
                        var value = -1;
                        ...
                        var row = $('#supplierGrid').jqxGrid('getrowdata', indexes);
                        $("#seq").val(row.seq);                        
                        $("#form1").submit();    
                    });
                    // delete row.
                    deleteButton.click(function (event) {
                        gridId = "supplierGrid";
                        deleteUrl = "Actions/SupplierAction.php?call=deleteSuppliers";
                        deleteSuppliers(gridId,deleteUrl);
                    });
                    exportButton.click(function (event) {
						filterQstr = getFilterString("supplierGrid");
						exportSuppliers(filterQstr);
                    });
                    reloadButton.click(function (event) {
                    	$("#supplierGrid").jqxGrid({ source: dataAdapter });
                    });
                }
            });
        }

        function exportSuppliers(filterString){
